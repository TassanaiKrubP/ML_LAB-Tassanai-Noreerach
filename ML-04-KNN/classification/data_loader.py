...
def load_data(data_path, img_size=32, max_per_class=None):
    """Read every class sub-directory under data_path.

    Returns images (n, h, w), labels (n,), class names, and file paths.
    """
    ...
    for label, class_name in enumerate(classes):
        ...
        for filename in filenames:
            ...
            images.append(image)
            labels.append(label)
            paths.append(os.path.join(class_name, filename))
            loaded += 1

        print(f"Loaded class {class_name}: {loaded} images ({skipped} skipped)")

    return np.stack(images), np.array(labels), classes, paths
